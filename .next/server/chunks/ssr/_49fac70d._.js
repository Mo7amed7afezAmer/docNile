...
"[project]/app/(dashboard)/processing/[batch_id]/page.tsx [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
        ...
        const fetchStatus = async ()=>{
            try {
                ...
                if (data.files) {
                    setFiles(data.files);
                    // Merge all logs
                    const allLogs = data.files.flatMap((f)=>f.logs?.map((log)=>`[${f.filename}] ${log}`));
                    setLogs(allLogs);
                }
            } catch (err) {
                console.error(err);
...
//# sourceMappingURL=_49fac70d._.js.map